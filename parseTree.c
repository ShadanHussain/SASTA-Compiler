...
ParseTreeNode *createNewParseTreeNode(tokenInfo *token, ParseTreeNode *parent, int variableNo)
{
...
ParseTree *generateParseTree(int **parseTable, Linkedlist **firstSets, Linkedlist **followSets, Linkedlist **grammarRules, HashmapEntry *variables, char *reverseMap[], int* nullables, int rule_no, int token_no, int epsilonTokenNo, FILE*fp)
{
    ...
    ParseTreeNode *curr = createNewParseTreeNode(NULL, NULL, search(variables, "<program'>"));
    parseTree->root = curr;
    push(st, curr);
        ...
        else
        {
            ...
            else
            {
                ...
                while (temp != NULL)
                {
                    ...
                    ParseTreeNode *newNode = createNewParseTreeNode(NULL, top, variableNo);
